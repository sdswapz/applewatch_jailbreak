...
uint64_t prep_payload() {
    int prealloc_size = 0x900;
    //alloc prealloc_port
    ...
    mach_port_t one_port = prealloc_port(prealloc_size);
    mach_port_t two_port = prealloc_port(prealloc_size);
...
// need to pass the kernel slide
int jail() {
    uint64_t kernel_base = prep_payload();
